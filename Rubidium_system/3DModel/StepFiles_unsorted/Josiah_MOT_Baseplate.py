...
base_dy = 6.5*layout.inch
base_dz = layout.inch
gap = layout.inch/8

# x-y coordinates of mount holes (in inches) (x,y)
mount_holes = [(0, 0), (0, 5.5), (9, 1.5), (9, 5.5)]
...
def example_baseplate(x=0, y=0, angle=0):
    ...
    # # add input fiberport, defined at the same coordinates as beam
    # baseplate.place_element("Input Fiberport2", optomech.fiberport_mount_km05,
    #                         x=8*gap, y=input_y, angle=layout.cardinal['right'])

    # baseplate.place_element("Input Fiberport", optomech.fiberport_mount_km05, x=1*layout.inch, y=input_y, angle=0)

    baseplate.place_element("DFB", optomech.Koheron_DFB_Laser, x=3*layout.inch, y=input_y, angle=0)
    ...
    baseplate.place_element_along_beam("Optical_Isolator", optomech.isolator_405, beam,
                                       beam_index=0b1, distance=1.125*layout.inch, angle=layout.cardinal['left'])
    ...
    baseplate.place_element_along_beam("Beam Splitter Cube", optomech.cube_splitter, beam,
                                       beam_index=0b1, distance=0.875*layout.inch, angle=layout.cardinal['right'],
    ...
    # add waveplate along the reflected beam, mounted in a rotation stage
    baseplate.place_element_along_beam("Rotation Stage", optomech.waveplate, beam,
                                       beam_index=0b11, distance=0.875*layout.inch, angle=layout.cardinal['left'],
                                       mount_type=optomech.rotation_stage_rsp05)
    ...
    # add mirror along the transmitted beam, mounted in a m05 mount
    baseplate.place_element_along_beam("Mirror", optomech.circular_mirror, beam,
                                       beam_index=0b10, distance=2*layout.inch, angle=layout.turn['down-left'],
                                       mount_type=optomech.mirror_mount_M05,
                                       mount_args=dict(thumbscrews=True))

    # add waveplate along the transmitted beam, mounted in a rotation stage
    baseplate.place_element_along_beam("Rotation Stage", optomech.waveplate, beam,
                                       beam_index=0b10, distance=0.75*layout.inch, angle=layout.cardinal['left'],
                                       mount_type=optomech.rotation_stage_rsp05)
    ...
    baseplate.place_element_along_beam("Output Fiberport", optomech.fiberport_mount_km05T_custom, beam,
                                       beam_index=0b10, distance=2.25*layout.inch, angle=layout.cardinal['right'],
                                       mount_args=dict(thumbscrews=True))
