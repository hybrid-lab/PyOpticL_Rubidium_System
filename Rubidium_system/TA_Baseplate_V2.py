...
def example_baseplate(x=0, y=0, angle=0):
    ...
    # add beam
    beam = baseplate.add_beam_path(x=3.385*layout.inch, y=6.5*layout.inch, angle=layout.cardinal['right'])

    baseplate.place_element("TA", optomech.TA_butterfly, x=3.5*layout.inch, y=6.5*layout.inch, angle=0)
    ...
    baseplate.place_element_along_beam("Optical_Isolator", optomech.isolator_850, beam,
                                       beam_index=0b1, distance=2.75*layout.inch, angle=layout.cardinal['left'])
    ...
    baseplate.place_element_along_beam("Output Fiberport", optomech.fiberport_mount_km05T, beam,
                                       beam_index=0b11, distance=4.75*layout.inch - 30.536, angle=layout.cardinal['right'],
                                       mount_args=dict(thumbscrews=True))
    ...
    baseplate.place_element_along_beam("Mirror", optomech.circular_mirror, beam,
                                       beam_index=0b10, distance=150-1.75*layout.inch, angle=layout.turn['up-left'],
                                       mount_type=optomech.mirror_mount_M05,
                                       mount_args=dict(thumbscrews=True))
    ...
    aom = baseplate.place_element_along_beam("AOM", optomech.AOMO_3100_125, beam,
                                       beam_index=0b10, distance=1.5*layout.inch, angle=layout.cardinal['up'],
    ...
    baseplate.place_element_along_beam("Mirror", optomech.circular_mirror, beam,
                                       beam_index=0b101, distance=3.25*layout.inch, angle=layout.turn['down-left'] + aom.DiffractionAngle.Value,
                                       mount_type=optomech.mirror_mount_M05,
                                       mount_args=dict(thumbscrews=True))
    ...
    lens = baseplate.place_element_along_beam("Lens f212mm B coat", optomech.circular_lens, beam,
                                         beam_index=0b101, distance=150-3.25*layout.inch, angle=layout.cardinal['right'] + aom.DiffractionAngle.Value,
                                         focal_length=160, part_number='AC127-075-B', mount_type=optomech.lens_holder_l05g)    
    ...
    #                                   mount_args=dict(thumbscrews=True))    
    baseplate.place_element_along_beam("Output Fiberport", optomech.fiberport_mount_km05T, beam,
                                       beam_index=0b101, distance=3*layout.inch - 17.836, angle=layout.cardinal['right'],
                                       mount_args=dict(thumbscrews=True))
