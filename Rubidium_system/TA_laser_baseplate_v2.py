...
def example_baseplate(x=0, y=0, angle=0):
    ...
    #holes for potential cylindrical lenses
    ...
    baseplate.place_element("TA", optomech.TA_butterfly, x=input_x, y=input_y, angle=0)

    baseplate.place_element_along_beam("Mirror", optomech.circular_mirror, beam,
                                   beam_index=0b1, distance=2*layout.inch, angle=layout.turn['up-left'],
                                   mount_type=optomech.mirror_mount_M05,
                                   mount_args=dict(thumbscrews=True))
    
    baseplate.place_element_along_beam("Mirror", optomech.circular_mirror, beam,
                                   beam_index=0b1, distance=5*layout.inch, angle=layout.turn['down-left'],
                                   mount_type=optomech.mirror_mount_M05,
                                   mount_args=dict(thumbscrews=True))
    ...
    baseplate.place_element_along_beam("Mirror", optomech.circular_mirror, beam,
                                       beam_index=0b11, distance=2.25*layout.inch, angle=layout.turn['down-right'],
                                       mount_type=optomech.mirror_mount_M05,
                                       mount_args=dict(thumbscrews=True))

    # add waveplate along the reflected beam, mounted in a rotation stage
    baseplate.place_element_along_beam("1/2 Waveplate", optomech.waveplate, beam,
                                       beam_index=0b11, distance=2.25*layout.inch, angle=layout.cardinal['right'],
                                       mount_type=optomech.rotation_stage_rsp05)
    ...
    baseplate.place_element_along_beam("Lens f212mm B coat", optomech.circular_lens, beam,
                                         beam_index=0b10, distance=.5*layout.inch, angle=layout.cardinal['down'],
                                         focal_length=150, part_number='AC127-050-B', mount_type=optomech.lens_holder_l05g)
    
    baseplate.place_element_along_beam("Mirror", optomech.circular_mirror, beam,
                                   beam_index=0b10, distance=4.75*layout.inch, angle=layout.turn['up-left'],
                                   mount_type=optomech.mirror_mount_M05,
                                   mount_args=dict(thumbscrews=True))
    ...
    aom = baseplate.place_element_along_beam("AOM", optomech.AOMO_3100_125, beam,
                                       beam_index=0b10, distance=150-4.75*layout.inch, angle=layout.cardinal['right'],
    ...
    baseplate.place_element_along_beam("Mirror", optomech.circular_mirror, beam,
                                       beam_index=0b101, distance=3.5*layout.inch, angle=layout.turn['up-right'] + aom.DiffractionAngle.Value,
                                       mount_type=optomech.mirror_mount_M05,
                                       mount_args=dict(thumbscrews=True))

    # Lens 2
    lens = baseplate.place_element_along_beam("Lens f212mm B coat", optomech.circular_lens, beam,
                                         beam_index=0b101, distance=150-3.5*layout.inch, angle=layout.cardinal['up'] + aom.DiffractionAngle.Value,
                                         focal_length=150, part_number='AC127-075-B', mount_type=optomech.lens_holder_l05g)    
    

        # add mirror along the transmitted beam, mounted in a m05 mount
    baseplate.place_element_along_beam("Mirror", optomech.circular_mirror, beam,
                                       beam_index=0b101, distance=1*layout.inch, angle=layout.turn['down-right'] + aom.DiffractionAngle.Value,
                                       mount_type=optomech.mirror_mount_M05,
                                       mount_args=dict(thumbscrews=True))
    ...
    baseplate.place_element_along_beam("Iris", optomech.pinhole_ida12, beam,
                                       beam_index=0b101, distance=2.5*layout.inch, angle=layout.cardinal['left'])

    baseplate.place_element_along_beam("1/2 Waveplate", optomech.waveplate, beam,
                                       beam_index=0b101, distance=0.75*layout.inch, angle=layout.cardinal['left'],
                                       mount_type=optomech.rotation_stage_rsp05)
